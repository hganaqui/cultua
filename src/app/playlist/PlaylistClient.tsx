...
export default function PlaylistClient() {
  ...
  function openCreate() {
    setEditItem(null); setFormTitle(''); setFormDesc(''); setFormPublic(false); setShowModal(true)
  }

  function openEdit(pl: Playlist) {
    setEditItem(pl); setFormTitle(pl.title); setFormDesc(pl.description ?? '')
    setFormPublic(pl.public); setShowModal(true)
  }
  ...
  async function handleSave() {
    ...
    if (editItem) {
      const { data } = await supabase
        .from('playlists')
        .update({ title: formTitle.trim(), description: formDesc.trim() || null, public: formPublic, updated_at: new Date().toISOString() })
        .eq('id', editItem.id).select().single()
      if (data) setPlaylists(prev => prev.map(p => p.id === editItem.id ? data as Playlist : p))
    } else {
      const { data } = await supabase
        .from('playlists')
        .insert({ user_id: userId, title: formTitle.trim(), description: formDesc.trim() || null, public: formPublic })
        .select().single()
  ...
  return (
    <main style={{
      ...
      minHeight: '100vh', backgroundColor: DS.colors.bg.primary,
    }}>
      ...
      <div style={{
        ...
        <button
          onClick={openCreate}
          style={{
            backgroundColor: DS.colors.primary.main, color: '#FFFFFF', border: 'none',
            padding: '10px 20px', borderRadius: DS.borderRadius.lg,
          ...
          {playlists.map(pl => (
            <div
              ...
              style={{
                ...
                borderRadius: DS.borderRadius.xl, padding: '20px',
                border: `1px solid ${DS.colors.neutral.light}`,
                display: 'flex', flexDirection: 'column' as const, gap: '12px',
                transition: DS.transitions.base, boxShadow: DS.shadows.sm,
              }}
              onMouseEnter={e => {
            ...
            >
              ...
              <div style={{
                ...
                fontSize: '24px', border: `1px solid ${DS.colors.primary.main}30`,
              }}>
                🎵
              </div>
              ...
              <div style={{ flex: 1 }}>
                <h3 style={{
                  fontFamily: DS.typography.fontFamily.heading,
                  fontSize: '16px', fontWeight: DS.typography.fontWeight.bold,
                ...
                {pl.description && (
                  <p style={{
                    fontFamily: DS.typography.fontFamily.body,
                    fontSize: '13px', color: DS.colors.text.secondary, lineHeight: 1.5,
                ...
                <div style={{ display: 'flex', gap: '8px', marginTop: '8px', flexWrap: 'wrap' as const }}>
                  <span style={{
                    fontFamily: DS.typography.fontFamily.body,
                    fontSize: '11px', color: DS.colors.text.muted,
                    backgroundColor: DS.colors.neutral.light,
                    padding: '3px 8px', borderRadius: DS.borderRadius.full,
                  }}>
                  ...
                  {pl.public && (
                    <span style={{
                      fontFamily: DS.typography.fontFamily.body,
                      fontSize: '11px', color: SUCCESS_COLOR,
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button
                  onClick={() => openEdit(pl)}
                  style={{
                    flex: 1, backgroundColor: DS.colors.bg.primary,
                    color: DS.colors.text.secondary,
                    border: `1.5px solid ${DS.colors.neutral.medium}`,
                    borderRadius: DS.borderRadius.md, padding: '8px',
                    fontFamily: DS.typography.fontFamily.body,
                ...
                <button
                  ...
                  style={{
                    backgroundColor: `${ERROR_COLOR}12`, color: ERROR_COLOR,
                    border: `1.5px solid ${ERROR_COLOR}35`,
                    borderRadius: DS.borderRadius.md, padding: '8px 12px',
                    fontFamily: DS.typography.fontFamily.body,
                    fontSize: '13px', cursor: deleteId === pl.id ? 'not-allowed' : 'pointer',
                    opacity: deleteId === pl.id ? 0.6 : 1,
                    transition: DS.transitions.fast,
                  }}
                  onMouseEnter={e => { if (deleteId !== pl.id) e.currentTarget.style.backgroundColor = `${ERROR_COLOR}22` }}
                  onMouseLeave={e => { if (deleteId !== pl.id) e.currentTarget.style.backgroundColor = `${ERROR_COLOR}12` }}
                >
                  {deleteId === pl.id ? '...' : '🗑️'}
                </button>
              </div>
      ...
      {showModal && (
        <>
          ...
          <div style={{
            ...
            width: '100%', maxWidth: '440px', zIndex: 301,
            ...
            <h2 style={{
              ...
              color: DS.colors.text.primary, marginBottom: '20px',
            }}>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <input
                ...
                placeholder="Ex: Louvores da Manhã" autoFocus
                style={inputStyle}
            ...
            <label style={{
              ...
              <input
                type="checkbox" checked={formPublic}
                onChange={e => setFormPublic(e.target.checked)}
                style={{ width: '16px', height: '16px', accentColor: DS.colors.primary.main }}
              />
              <span style={{ fontFamily: DS.typography.fontFamily.body, fontSize: '14px', color: DS.colors.text.secondary }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button
                ...
                style={{
                  ...
                  color: DS.colors.text.secondary,
                  border: `1.5px solid ${DS.colors.neutral.medium}`,
                  borderRadius: DS.borderRadius.lg, padding: '12px',
                  fontFamily: DS.typography.fontFamily.body,
                  fontSize: '14px', fontWeight: DS.typography.fontWeight.medium,
                  cursor: 'pointer', transition: DS.transitions.fast,
                }}
              ...
              <button
                ...
                onMouseEnter={e => { if (formTitle.trim() && !saving) e.currentTarget.style.backgroundColor = DS.colors.primary.light }}
                onMouseLeave={e => { if (formTitle.trim() && !saving) e.currentTarget.style.backgroundColor = DS.colors.primary.main }}
              >
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '16px' }}>
      ...
      {[1, 2, 3].map(i => (
        <div key={i} style={{
          backgroundColor: DS.colors.bg.secondary, borderRadius: DS.borderRadius.xl,
          padding: '20px', border: `1px solid ${DS.colors.neutral.light}`,
          ...
          <div style={{ height: '18px', backgroundColor: DS.colors.neutral.medium, borderRadius: DS.borderRadius.sm, width: '70%', marginBottom: '8px' }} />
          <div style={{ height: '14px', backgroundColor: DS.colors.neutral.medium, borderRadius: DS.borderRadius.sm, width: '90%' }} />
        </div>
      ))}
    </div>
  )
